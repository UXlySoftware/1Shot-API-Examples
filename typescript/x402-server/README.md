...
- [1Shot API Key & Secret](https://app.1shotapi.com/api-keys) for accepting payments on a mainnet or testnet
- [1Shot API server wallet](https://app.1shotapi.com/wallets) for relaying payment transactions
  -- be sure to put sufficient gas funds into the server wallet to pay for transactions
...
### Successful Response
```ts
// Bodyld all packages from the typescript examples root:
```bash
cd ../../
pnpm install
pnpm build
